...
HTTP_TIMEOUT = 1     # HTTP请求超时时间
PING_TIMEOUT = 1     # Ping超时时间
...
def test_http_port(ip, port):
    """测试指定IP和端口的HTTP响应 (增强错误处理)"""
    try:
        ...
        return {
            ...
            'content': response.text[:2000],  # 只保存前2000字符
            'url': url,
            'success': True
        }
    except requests.exceptions.Timeout:
        return {'error': '请求超时', 'url': url, 'success': False}
    except requests.exceptions.SSLError:
        return {'error': 'SSL证书错误', 'url': url, 'success': False}
    except requests.exceptions.ConnectionError:
        return {'error': '连接失败', 'url': url, 'success': False}
